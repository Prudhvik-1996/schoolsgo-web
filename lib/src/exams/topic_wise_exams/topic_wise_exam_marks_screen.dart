...
class TopicWiseExamMarksScreen extends StatefulWidget {
  const TopicWiseExamMarksScreen({
    Key? key,
  ...
  final TopicWiseExam topicWiseExam;
  final Future<void> Function(List<StudentExamMarks> marksList) updateExamMarks;

  @override
  State<TopicWiseExamMarksScreen> createState() => _TopicWiseExamMarksScreenState();
}
  ...
  Future<void> _loadData() async {
    ...
    setState(() {
      studentsList = <StudentProfile>{
        ...
        ...widget.studentsList.where((e) => (widget.topicWiseExam.studentExamMarksList ?? []).map((e) => e?.studentId).contains(e.studentId))
      }.toList();
      ...
      for (StudentProfile eachStudent in studentsList) {
        StudentExamMarks? actualMarksBean =
            (widget.topicWiseExam.studentExamMarksList ?? []).where((e) => e?.studentId == eachStudent.studentId).firstOrNull;
        if (actualMarksBean != null) {
          examMarks.add(StudentExamMarks.fromJson(actualMarksBean.toJson()));
        } else {
          examMarks.add(StudentExamMarks(
            examSectionSubjectMapId: widget.topicWiseExam.examSectionSubjectMapId,
            examId: widget.topicWiseExam.examId,
            ...
            children: [
              Row(
                children: [
                  Expanded(
                    child: Text(
                      widget.topicWiseExam.topicName ?? "-",
              ...
              Row(
                children: [
                  Expanded(
                    child: Text("Max. Marks: ${widget.topicWiseExam.maxMarks ?? "-"}"),
                  ),
                  const SizedBox(width: 15),
                  Expanded(child: Text("Class Average: ${widget.topicWiseExam.classAverage ?? "-"}")),
                ],
              ),
              const SizedBox(height: 15),
              Row(
                children: [
                  Expanded(
                    child: Text("Date: ${widget.topicWiseExam.examDate}"),
                  ),
                  const SizedBox(width: 15),
                  Expanded(
                    child: Text("Time: ${widget.topicWiseExam.examTimeSlot}"),
                  ),
                ],
              ),
            ],
