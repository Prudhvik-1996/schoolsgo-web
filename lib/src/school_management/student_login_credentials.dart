      ...
      child: Scrollbar(
        ...
        child: SingleChildScrollView(
          ...
          child: DataTable(
            ...
            rows: [
              ...studentsList.where((StudentProfile es) => es.sectionId == selectedSection.sectionId).map(
                    (es) => DataRow(
                      cells: [
                        DataCell(
                          placeholder: true,
  ...
  onSortColum(int columnIndex, bool ascending) {
    if (columnIndex == 0) {
      if (ascending) {
        ...
        studentsList.sort((a, b) => (int.tryParse(b.rollNumber ?? "") ?? 0).compareTo((int.tryParse(a.rollNumber ?? "") ?? 0)));
      }
    } else if (columnIndex == 1) {
      if (ascending) {
        studentsList.sort((a, b) => (a.studentFirstName ?? "").compareTo((b.studentFirstName ?? "")));
      } else {
        studentsList.sort((a, b) => (b.studentFirstName ?? "").compareTo((a.studentFirstName ?? "")));
      }
    } else if (columnIndex == 2) {
      if (ascending) {
        studentsList.sort((a, b) => (a.gaurdianFirstName ?? "").compareTo((b.gaurdianFirstName ?? "")));
      } else {
        studentsList.sort((a, b) => (b.gaurdianFirstName ?? "").compareTo((a.gaurdianFirstName ?? "")));
      }
    } else if (columnIndex == 3) {
      if (ascending) {
        studentsList.sort((a, b) => (a.gaurdianMobile ?? "").compareTo((b.gaurdianMobile ?? "")));
      } else {
        studentsList.sort((a, b) => (b.gaurdianMobile ?? "").compareTo((a.gaurdianMobile ?? "")));
      }
    }
