...
class MegaAdminCircularsScreen extends StatefulWidget {
  ...
  @override
  State<MegaAdminCircularsScreen> createState() => _MegaAdminCircularsScreenState();
}

class _MegaAdminCircularsScreenState extends State<MegaAdminCircularsScreen> {
  ...
  Future<void> _loadData() async {
    setState(() {
      ...
      newCircular = CircularBean(
        ...
        schoolName: widget.megaAdminProfile.schoolName,
        status: "active",
        circularMediaBeans: [],
        agentId: widget.megaAdminProfile.userId,
        circularType: "A",
      )..isEditMode = true;
                  ...
                  children: [
                    ...
                    Expanded(
                      child: ScrollablePositionedList.builder(
                        ...
                        itemCount:
                            (isAddNew ? [newCircular] : circulars.where((e) => selectedSchool == null || selectedSchool!.schoolId == e.schoolId))
                        ...
                        itemBuilder: (context, index) => Container(
                          margin: MediaQuery.of(context).orientation == Orientation.landscape
                              ? EdgeInsets.fromLTRB(MediaQuery.of(context).size.width / 4, 20, MediaQuery.of(context).size.width / 4, 20)
                              : const EdgeInsets.all(20),
                          child: (isAddNew ? [newCircular] : circulars.where((e) => selectedSchool == null || selectedSchool!.schoolId == e.schoolId))
                                  .where((e) => e.status == "active")
                                  .toList()[index]
                                  .isEditMode
                              ? _buildCircularsWidgetForEditMode((isAddNew
                                      ? [newCircular]
                                      : circulars.where((e) => selectedSchool == null || selectedSchool!.schoolId == e.schoolId))
                                  .where((e) => e.status == "active")
                                  .toList()[index])
                              : _buildCircularsWidgetForReadMode((isAddNew
                                      ? [newCircular]
                                      : circulars.where((e) => selectedSchool == null || selectedSchool!.schoolId == e.schoolId))
                                  .where((e) => e.status == "active")
                                  .toList()[index]),
              ...
              Container(
                ...
                child: Row(
                  ...
                  children: [
                    Expanded(
                      child: Text(
                        circular.schoolName == null
                            ? circular.franchiseName!
                            : circular.branchCode == null
                                ? circular.schoolName!
                                : (circular.schoolName! + " - " + circular.branchCode!),
                        style: TextStyle(
                          color: circular.schoolName == null ? Colors.blue : null,
                        ),
                      ),
                    ),
                    Text(
