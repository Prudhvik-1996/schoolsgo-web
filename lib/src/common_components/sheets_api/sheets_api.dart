import 'package:gsheets/gsheets.dart';

class SheetsApi {
  static const _credentials = r'''
{
  "type": "service_account",
  "project_id": "schoolsgo-283018",
  "private_key_id": "e7ccb4c3ea33777540783ab3bc9fc97a9c5c3d6d",
  "private_key": "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
  "client_email": "epsilon-diary-sheets@schoolsgo-283018.iam.gserviceaccount.com",
  "client_id": "116884205406396008309",
  "auth_uri": "https://accounts.google.com/o/oauth2/auth",
  "token_uri": "https://oauth2.googleapis.com/token",
  "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs",
  "client_x509_cert_url": "https://www.googleapis.com/robot/v1/metadata/x509/epsilon-diary-sheets%40schoolsgo-283018.iam.gserviceaccount.com"
}
''';

  static final _spreadSheetId = '1uhYQ_zOVRInAzd0SM-pS2O4C8JBawaQwD0QlE4OgFok';

  static final _gsheets = GSheets(_credentials);

  static Worksheet? testSheet;

  static Future init(String sheetId) async {
    final spreadSheet = await _gsheets.createSpreadsheet(
      "New Spread Sheet",
      worksheetTitles: ["Sheet1"],
    );
    try {
      testSheet = await spreadSheet.worksheetByTitle("Sheet1");
      print("32: ${testSheet?.spreadsheetId}");
    } catch (e) {
      print("33: $e");
    }
    try {
      testSheet!.values.insertRow(1, ["Admin Name", "Expense Type", "..."]);
    } catch (e) {
      print("38: $e");
    }
    print("docs.google.com/feeds/download/spreadsheets/Export?key=$sheetId&exportFormat=xlsx");
    // await launchUrl(Uri.parse("docs.google.com/feeds/download/spreadsheets/Export?key=$sheetId&exportFormat=xlsx"));
    // await downloadFile(
    //   "docs.google.com/feeds/download/spreadsheets/Export?key=$sheetId&exportFormat=xlsx",
    // );
  }
}
