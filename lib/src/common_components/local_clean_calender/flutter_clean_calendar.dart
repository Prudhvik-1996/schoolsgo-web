...
class CalendarState extends State<Calendar> {
  ...
  void reload(Map<DateTime, List<CleanCalendarEvent>>? newEvents) => setState(() {
        events = newEvents;
        _selectedEvents = events?[DateTime(widget.selectedDate.year, widget.selectedDate.month, widget.selectedDate.day)] ?? [];
      });
  ...
  void initState() {
    ...
    selectedMonthsDays = _daysInMonth(widget.selectedDate);
    selectedWeekDays = Utils.daysInRange(_firstDayOfWeek(widget.selectedDate), _lastDayOfWeek(widget.selectedDate)).toList();
    initializeDateFormatting(widget.locale, null).then((_) => setState(() {
          var monthFormat = DateFormat('MMMM yyyy', widget.locale).format(widget.selectedDate);
        ...
        if (this.widget.dayBuilder != null) {
          ...
          dayWidgets.add(
            CalendarTile(
              ...
              isSelected: Utils.isSameDay(widget.selectedDate, day),
              inMonth: day.month == widget.selectedDate.month,
            ),
          );
        }
      },
        ...
        child: Container(
          ...
          child: Row(
            ...
            children: <Widget>[
              ...
              Text(
                DateFormat(widget.expandableDateFormat, widget.locale).format(widget.selectedDate),
  ...
  void resetToToday() {
    ...
    var firstDayOfCurrentWeek = _firstDayOfWeek(widget.selectedDate);
    var lastDayOfCurrentWeek = _lastDayOfWeek(widget.selectedDate);
    ...
    _launchDateSelectionCallback(widget.selectedDate);
  }

  void nextMonth() {
    setState(() {
      widget.selectedDate = Utils.nextMonth(widget.selectedDate);
      var firstDateOfNewMonth = Utils.firstDayOfMonth(widget.selectedDate);
      var lastDateOfNewMonth = Utils.lastDayOfMonth(widget.selectedDate);
  ...
  void previousMonth() {
    setState(() {
      widget.selectedDate = Utils.previousMonth(widget.selectedDate);
      var firstDateOfNewMonth = Utils.firstDayOfMonth(widget.selectedDate);
      var lastDateOfNewMonth = Utils.lastDayOfMonth(widget.selectedDate);
      updateSelectedRange(firstDateOfNewMonth, lastDateOfNewMonth);
      selectedMonthsDays = _daysInMonth(widget.selectedDate);
  ...
  void nextWeek() {
    setState(() {
      widget.selectedDate = Utils.nextWeek(widget.selectedDate);
      var firstDayOfCurrentWeek = _firstDayOfWeek(widget.selectedDate);
      var lastDayOfCurrentWeek = _lastDayOfWeek(widget.selectedDate);
  ...
  void previousWeek() {
    setState(() {
      widget.selectedDate = Utils.previousWeek(widget.selectedDate);
      var firstDayOfCurrentWeek = _firstDayOfWeek(widget.selectedDate);
      var lastDayOfCurrentWeek = _lastDayOfWeek(widget.selectedDate);
      updateSelectedRange(firstDayOfCurrentWeek, lastDayOfCurrentWeek);
      selectedWeekDays = Utils.daysInRange(firstDayOfCurrentWeek, lastDayOfCurrentWeek).toList();
  ...
  void handleSelectedDateAndUserCallback(DateTime day) {
    ...
    if (widget.selectedDate.month > day.month) {
      // Day in next year selected? Switch to next month.
      if (widget.selectedDate.year < day.year) {
        nextMonth();
      } else {
        previousMonth();
      }
    ...
    setState(() {
      ...
      _selectedEvents = events?[widget.selectedDate] ?? [];
    });
  ...
  _firstDayOfWeek(DateTime date) {
    var day = DateTime.utc(widget.selectedDate.year, widget.selectedDate.month, widget.selectedDate.day, 12);
