...
class AdminManageFeeTypesScreen extends StatefulWidget {
  const AdminManageFeeTypesScreen({Key? key, required this.adminProfile})
      : super(key: key);

  final AdminProfile adminProfile;

  @override
  _AdminManageFeeTypesScreenState createState() =>
      _AdminManageFeeTypesScreenState();
}
...
class _AdminManageFeeTypesScreenState extends State<AdminManageFeeTypesScreen> {
  ...
  Future<void> _loadData() async {
    ...
    GetFeeTypesResponse getFeeTypesResponse =
        await getFeeTypes(getFeeTypesRequest);

    if (getFeeTypesResponse.httpStatus == "OK" &&
        getFeeTypesResponse.responseStatus == "success") {
      setState(() {
    ...
    if (getFeeMapResponse.httpStatus == "OK" &&
        getFeeMapResponse.responseStatus == "success") {
      setState(() {
  ...
  Widget _getCustomFeeTypeWidget(
      CustomFee customFee, FeeType feeType, Section section) {
    List<FeeMap> _feeMaps = feeMaps
        .where((eachFeeMap) =>
            eachFeeMap.sectionId == section.sectionId &&
            eachFeeMap.customFeeId != null)
        .toList();
    if (_feeMaps.isEmpty) return Container();
    print(
        "${section.sectionName} ${feeType.feeType} ${customFee.customFeeDescription} ${_feeMaps.length}");
        ...
        children: [
          Row(
            children: [
              ...
              Text(((feeMap.feeMapAmount ??
                          feeMap.customFeeAmount ??
                          feeMap.feeMapAmount ??
                          0) /
                      100.0)
                  .toString()),
  ...
  Widget _getFeeTypeWidget(FeeType feeType, Section section) {
    List<FeeMap> _feeMaps = feeMaps
        .where((eachFeeMap) => eachFeeMap.sectionId == section.sectionId)
        .toList();
    if (_feeMaps.isEmpty) return Container();
        ...
        children: <Widget>[
              Row(
                children: [
                  ...
                  Text(
                    ((feeMap.feeMapAmount ?? feeMap.feeTypeAmount ?? 0) / 100.0)
                        .toString(),
                  ),
                ],
              ),
